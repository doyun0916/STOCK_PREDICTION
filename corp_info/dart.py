...
for i in range(1, len(corp_name)):
    if corp_name[i] == "]":
        ...
        break
start = str(datetime.now() + timedelta(days=-365))[:10]          # (-365, 즉 지금으로부터 1년전 공시자료부터 최신까지 검색을 위한 기간 설정)
...
for i in range(1, 3):
    ...
    for i in range(len(result)):                                                    # 검색 결과를 제목만 뽑아내기
        titles.append(result.report_list[i].report_nm)

titles = pd.DataFrame(titles)
titles.to_csv('./gongsi_titles.csv', sep=',', na_rep='NaN', index=False)
